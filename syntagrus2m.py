...
class RuscorporaSynTagRus2CONLL(xml.sax.handler.ContentHandler):
    ...
    def __flush_word(self):
        ...
        for key in self.__word_features:
            ...
            if key == 'FEAT' and not WRITE_GRAMMAR_TAGS:
                self.__word_features[key] = self.__word_features[key].split(',')[0]
        lemma = "_"
        DUAL_TABS = u"{id}\t{form}\t{lemma}\t{dr}{num}{gen}\t{df}\t{case}|{gen}|{num}|{tense}\t{head}\t{deprel}"
    ...
    u'nid': 'nid',
        ...
        wv = self.map_to_key(wfs, "df", wv, dform) 
        wv = self.map_to_key(wfs, "dr", wv, dform)
        if u'срав' in wfs or u'смяг' in wfs: wv["dr"]="comp"
        if u'прич' in wfs: wv["dr"]="a"
        if u'деепр' in wfs: wv["dr"]="adv"
        if u'инф'in wfs: wv["dr"]="n"#v1.1
        #if u'v' in wfs or u'part' in wfs or u'pr' in wfs or u"conj" in wfs or u"nid" in wfs: wv["lemma"]=self.__word_features.get('LEMMA', '-')
        wv["lemma"]=self.__word_features.get('LEMMA', '-') #v1.3
        string_to_flush = DUAL_TABS.format(**wv)
        print >>self.__out, string_to_flush
